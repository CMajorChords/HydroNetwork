...
class TiDEModel(nn.Module):
    ...
    def __init__(self, sizes, args):
        ...
        self.concat_shape = self.label_len * self.lookback_shape[1] + self.attr_shape[0] * self.attr_shape[1] + \
                            self.dynCov_shape[0] * self.feat_size
        # modules
        self.featproj = ResidualBlock(self.dynCov_shape[1], self.hidden_size, self.feat_size, self.drop_prob,
                                      self.layer_norm)
        self.encoder = TiDEEncoder(self.concat_shape, self.hidden_size, self.hidden_size, self.drop_prob,
                                   self.num_encoder_layers, self.layer_norm)
        self.denseDecoder = TiDEDenseDecoder(self.hidden_size, self.hidden_size,
                                             self.decoder_output_dim * self.pred_len, self.drop_prob,
                                             self.num_decoder_layers, self.layer_norm)
        self.temporalDecoder = TiDETemporalDecoder(self.feat_size + self.decoder_output_dim,
                                                   self.temporal_decoder_hidden, self.lookback_shape[1], self.drop_prob,
                                                   self.layer_norm)
    ...
    def forward(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        # batch_y_lookback = rearrange(batch_y_lookback,
        #                              'b l n -> (b n) l')  # [B,L,N] -> [B*N,L] Guarantee channel independence
        batch_y_lookback
        # lookback residual
